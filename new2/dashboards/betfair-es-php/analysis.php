    ...
    <div class="mx-auto max-w-7xl px-4 py-6 sm:px-6">
        ...
        <div class="mb-6">
            <div class="flex items-center gap-2">
                <h1 class="text-lg font-semibold text-text sm:text-xl">Análisis por Categoría</h1>
        ...
        <div class="mb-6 rounded-lg border border-border bg-surface p-4 sm:p-6">
            <h2 class="mb-4 text-sm font-medium text-text">Distribución por Categoría</h2>
                        ...
                        <div class="h-6 rounded bg-border/30 overflow-hidden" style="width: <?php echo $totalWidth; ?>%">
                            <div class="h-full flex">
                                <?php if ($cat['critical'] > 0): ?>
                                    <div class="h-full bg-red-500" style="width: <?php echo $criticalWidth; ?>%" title="Críticos: <?php echo $cat['critical']; ?>"></div>
                                <?php endif; ?>
                                <?php if ($cat['warning'] > 0): ?>
                                    <div class="h-full bg-amber-500" style="width: <?php echo $warningWidth; ?>%" title="Advertencias: <?php echo $cat['warning']; ?>"></div>
                                <?php endif; ?>
                                <?php if ($cat['opportunity'] > 0): ?>
                                    <div class="h-full bg-green-500" style="width: <?php echo $opportunityWidth; ?>%" title="Oportunidades: <?php echo $cat['opportunity']; ?>"></div>
                                <?php endif; ?>
                            </div>
                        </div>
                    </div>
            ...
            <div class="mt-6 flex justify-center gap-4">
                <div class="flex items-center gap-1.5">
                    ...
                    <span class="text-xs text-text-muted">Crítico</span>
                </div>
                <div class="flex items-center gap-1.5">
                    <span class="h-3 w-3 rounded bg-amber-500"></span>
                    <span class="text-xs text-text-muted">Advertencia</span>
                </div>
                <div class="flex items-center gap-1.5">
                ...
                <details class="rounded-lg border border-border bg-surface group">
                    <summary class="flex w-full cursor-pointer items-center justify-between p-4 text-left list-none">
                        <div>
                            ...
                            <p class="text-xs text-text-muted"><?php echo $cat['total']; ?> registros</p>
                        </div>
                        <div class="flex items-center gap-4">
                            ...
                            <div class="hidden items-center gap-3 sm:flex">
                                <span class="text-xs">
                                    ...
                                    <span class="text-text-muted"> crítico</span>
                                </span>
                                <span class="text-xs">
                                    <span class="text-amber-500"><?php echo $cat['warning']; ?></span>
                                    <span class="text-text-muted"> advertencia</span>
                                </span>
                                <span class="text-xs">
                                    <span class="text-green-500"><?php echo $cat['opportunity']; ?></span>
                                    <span class="text-text-muted"> oportunidad</span>
                                </span>
                            </div>
                    ...
                    <div class="border-t border-border p-4">
                        ...
                        <?php if (!empty($topTriggers)): ?>
                            <div class="mb-4">
                                <span class="text-xs text-text-muted">Triggers principales: </span>
                        ...
                        <a href="list.php?category=<?php echo $cat['id']; ?>" class="mt-3 inline-block text-xs text-text-muted hover:text-text">
                            Ver todos los registros de <?php echo htmlspecialchars($cat['name']); ?> →
                        </a>
                    </div>
                </details>
            <?php endforeach; ?>
