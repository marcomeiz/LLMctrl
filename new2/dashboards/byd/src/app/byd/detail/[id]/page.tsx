...
export default function BydDetailPage() {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === 'ArrowLeft' && prevRecord) {
        router.push(`/byd/detail/${prevRecord.id}`);
      } else if (e.key === 'ArrowRight' && nextRecord) {
        router.push(`/byd/detail/${nextRecord.id}`);
      }
    };
      ...
      <div className="flex min-h-screen flex-col bg-background">
        ...
        <main className="flex flex-1 items-center justify-center">
          <div className="text-center">
            ...
            <Link href="/byd/list" className="mt-4 inline-block text-sm text-text hover:underline">
              ← Back to list
            </Link>
          </div>
        </main>
          ...
          <div className="rounded-lg border border-border bg-surface">
            ...
            <div className="grid gap-4 border-b border-border p-4 sm:grid-cols-2 sm:p-6">
              ...
              <div>
                ...
                {(() => {
                  const pos = record.ranking_list?.findIndex(b => b.toLowerCase() === 'byd');
