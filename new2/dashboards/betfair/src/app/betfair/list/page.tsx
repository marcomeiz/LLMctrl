...
import { searchRecords, getCategories, Classification, CategoryId } from '@/lib/data';
...
export default function BetfairListPage() {
  ...
  const filteredRecords = useMemo(() => {
    return searchRecords(query, { classification, category }, market);
  }, [query, classification, category, market]);

  const stats = useMemo(() => {
    const all = searchRecords('', {}, market);
          ...
          <div className="mb-4 space-y-3">
            ...
            <div className="flex flex-wrap gap-2">
              {CATEGORIES.map((cat) => (
                ...
                >
                  {cat === 'ALL' ? 'All categories' : categoryNames.find(c => c.id === cat)?.name}
                </button>
              ))}
            </div>
          </div>
