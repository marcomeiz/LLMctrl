...
export default function BetfairDetailPage() {
  ...
  const fullAnswer = getFullAnswer(id, market);

  // Find prev/next records
  const currentIndex = marketRecords.findIndex(r => r.id === id);
  const prevRecord = currentIndex > 0 ? marketRecords[currentIndex - 1] : null;
  const nextRecord = currentIndex < marketRecords.length - 1 ? marketRecords[currentIndex + 1] : null;
            ...
            <div className="grid gap-4 border-b border-border p-4 sm:grid-cols-2 sm:p-6">
              ...
              {record.ranking_list && record.ranking_list.length > 0 && (
                <div className="sm:col-span-2">
                  <span className="text-xs text-text-muted">Full Ranking (competitors)</span>
